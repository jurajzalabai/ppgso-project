  ...
  void bezierShape(int count) {
    for(int i = 1; i < (int) controlPoints.size(); i+=3) {
      for (int j = 0; j <= count; j++) {
        ...
        glm::vec2 point;
        point = bezierPoint(controlPoints[i-1], controlPoints[i], controlPoints[i+1], controlPoints[i+2], j/float(count));
        points.emplace_back(point, 0);
        /* points.emplace_back(glm::vec2{0.5, 0.5}, 0);
        points.emplace_back(glm::vec2{0.8, 0.6}, 0);*/
